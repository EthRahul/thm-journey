# 📝 Cybersecurity Notes — Rahul Sunouri

> Personal notes and cheatsheets from my TryHackMe journey.
> Concise revision references covering tools, concepts and techniques.

[![TryHackMe](https://img.shields.io/badge/TryHackMe-Ethrahul-red?style=for-the-badge&logo=tryhackme)](https://tryhackme.com/p/Ethrahul)

---

## 🗂️ Index

- [📁 Cyber Security 101](#-cyber-security-101)
  - [Exploitation Basics](#exploitation-basics)
  - [Web Hacking](#web-hacking)
  - [Offensive Security Tooling](#offensive-security-tooling)
  - [OWASP Top 10](#owasp-top-10)
- [📁 Jr Penetration Tester](#-jr-penetration-tester)
  - [Introduction to Pentesting](#introduction-to-pentesting)
  - [Introduction to Web Hacking](#introduction-to-web-hacking)
  - [Burp Suite](#burp-suite)
  - [Network Security](#network-security)
  - [Vulnerability Research](#vulnerability-research)
  - [Privilege Escalation](#privilege-escalation)
- [📁 Web Application Pentesting](#-web-application-pentesting-upcoming)

---

## 📁 Cyber Security 101

### Exploitation Basics
| Room | Notes |
|---|---|
| Metasploit Introduction | [View](CyberSec-101/Exploitation-Basics/Metasploit-Introduction.md) |
| Metasploit Exploitation | [View](CyberSec-101/Exploitation-Basics/Metasploit-Exploitation.md) |
| Metasploit Meterpreter | [View](CyberSec-101/Exploitation-Basics/Metasploit-Meterpreter.md) |
| Blue — MS17-010 EternalBlue | [View](CyberSec-101/Exploitation-Basics/Blue.md) |
| Moniker Link CVE-2024-21413 | [View](CyberSec-101/Exploitation-Basics/Moniker-Link.md) |

### Web Hacking
| Room | Notes |
|---|---|
| Web Application Basics | [View](CyberSec-101/Web-Hacking/Web-Application-Basics.md) |
| JavaScript Essentials | [View](CyberSec-101/Web-Hacking/JavaScript-Essentials.md) |
| SQL Fundamentals | [View](CyberSec-101/Web-Hacking/SQL-Fundamentals.md) |
| Burp Suite Basics | [View](CyberSec-101/Web-Hacking/Burp-Suite-Basics.md) |

### Offensive Security Tooling
| Room | Notes |
|---|---|
| Hydra | [View](CyberSec-101/Offensive-Security-Tooling/Hydra.md) |
| Gobuster Basics | [View](CyberSec-101/Offensive-Security-Tooling/Gobuster-Basics.md) |
| Shells Overview | [View](CyberSec-101/Offensive-Security-Tooling/Shells-Overview.md) |
| SQLMap Basics | [View](CyberSec-101/Offensive-Security-Tooling/SQLMap-Basics.md) |

### OWASP Top 10
| Room | Notes |
|---|---|
| IAAA Failures | [View](CyberSec-101/OWASP-Top-10/OWASP-Top10-IAAA-Failures.md) |
| Application Design Flaws | [View](CyberSec-101/OWASP-Top-10/OWASP-Top10-Application-Design-Flaws.md) |
| Insecure Data Handling | [View](CyberSec-101/OWASP-Top-10/OWASP-Top10-Insecure-Data-Handling.md) |

---

## 📁 Jr Penetration Tester

### Introduction to Pentesting
| Room | Notes |
|---|---|
| Pentesting Fundamentals | [View](Jr-Pentesting-Path/Introduction-to-Pentesting/Pentesting-Fundamentals.md) |
| Principles of Security | [View](Jr-Pentesting-Path/Introduction-to-Pentesting/Principles-of-Security.md) |

### Introduction to Web Hacking
| Room | Notes |
|---|---|
| Walking An Application | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/Walking-An-Application.md) |
| Content Discovery | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/Content-Discovery.md) |
| Subdomain Enumeration | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/Subdomain-Enumeration.md) |
| Authentication Bypass | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/Authentication-Bypass.md) |
| IDOR | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/IDOR.md) |
| File Inclusion | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/File-Inclusion.md) |
| Intro to SSRF | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/SSRF.md) |
| XSS Introduction | [View](Jr-Pentesting-Path/Introduction-to-Web-Hacking/XSS-Intro.md) |

### Burp Suite
| Room | Notes |
|---|---|
| Burp Suite Repeater | [View](Jr-Pentesting-Path/Burp-Suite/Burp-Suite-Repeater.md) |
| Burp Suite Intruder | [View](Jr-Pentesting-Path/Burp-Suite/Burp-Suite-Intruder.md) |
| Burp Suite Other Modules | [View](Jr-Pentesting-Path/Burp-Suite/Burp-Suite-Other-Modules.md) |

### Network Security
| Room | Notes |
|---|---|
| Protocols and Servers 1 & 2 | [View](Jr-Pentesting-Path/Network-Security/Protocols-and-Servers-Combined.md) |
| Nmap Post Port Scans | [View](Jr-Pentesting-Path/Network-Security/Nmap-Post-Port-Scans.md) |
| Net Sec Challenge | [View](Jr-Pentesting-Path/Network-Security/Net-Sec-Challenge.md) |

### Vulnerability Research
| Room | Notes |
|---|---|
| Vulnerability Capstone | [View](Jr-Pentesting-Path/Vulnerability-Research/Vulnerability-Capstone.md) |

### Privilege Escalation
| Room | Notes |
|---|---|
| Linux Privilege Escalation | [View](Jr-Pentesting-Path/Privilege-Escalation/Linux-Privilege-Escalation.md) |
| Linux Privilege Escalation Capstone | [View](Jr-Pentesting-Path/Privilege-Escalation/Linux-Privilege-Escalation-Capstone.md) |

---

## 📁 Web Application Pentesting *(Upcoming)*

> 🔄 Starting next — focused on deep web app offensive techniques.

| Module | Status |
|---|---|
| Web Reconnaissance | ⏳ |
| Attacking Web Applications | ⏳ |
| Advanced SQL Injection | ⏳ |
| XSS & CSRF | ⏳ |
| API Security Testing | ⏳ |
| File Upload Vulnerabilities | ⏳ |

---

> *"The more you sweat in training, the less you bleed in battle."*
